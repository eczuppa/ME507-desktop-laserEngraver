 ...
 *           In general the class works by counting pulses from the qudarature encoder on a brushed DC motor by leveraging the STM32 Generic
...
class Quad_Encoder
{
    ...
    uint8_t _enc_sigpin_A;     ///< the pin that is recieving the A output signal of the encoder and is tied to the first timer channel
    uint8_t _enc_sigpin_B;     ///< the pin that is recieving the B output signal of the encoder and is tied to the second timer channel
    ...
    float enc_d_pos (void);        // relies on the encoder read method but the output in the change in linear displacement of the belt due to the motor's rotaiton since the last measurement
